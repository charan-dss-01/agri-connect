...
const ProductCard = ({ product, index }) => {
    ...
    const handleAddToCart = (e) => {
        e.preventDefault(); // Prevents triggering the outer Link
        navigate(`/cart/${_id}`);
    };
    ...
    return (
        <Link to={`/product/${product._id}`} key={_id}>
            <div
                className={`bg-gray-100 border border-gray-300 rounded-lg hover:shadow-lg overflow-hidden transform hover:scale-105 transition-transform duration-300 mx-2 animate__animated animate__fadeInLeft`}
    ...
    return (
        <div className="container mx-auto my-10 p-6">
            ...
            <Carousel responsive={responsive}>
                {/* Products Section */}
                {vegetableProducts && vegetableProducts.length > 0 ? (
